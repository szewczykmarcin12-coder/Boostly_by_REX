...
export default function PinScreen({ onSuccess, onAdminAccess }) {
  ...
  const handleSubmit = async (fullPin) => {
    ...
    setError('');
    try {
      const valid = await verifyPin(fullPin, mode);
      if (valid) {
        if (mode === 'admin') {
        ...
        triggerError('Nieprawidłowy PIN');
      }
    } catch (e) {
      triggerError('Błąd połączenia z serwerem');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-orange-50 flex flex-col items-center justify-center p-6">
      ...
      <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-6 ${mode === 'admin' ? 'bg-red-100' : 'bg-orange-100'}`}>
        {mode === 'admin' ? <Shield className="w-8 h-8 text-red-500" /> : <Lock className="w-8 h-8 text-primary" />}
      </div>
      ...
      <p className="text-gray-500 text-sm mb-8 text-center">
        {mode === 'admin' ? 'Wprowadź PIN administratora' : 'Wprowadź 6-cyfrowy PIN aby uzyskać dostęp'}
      </p>
        ...
        {pin.map((digit, index) => (
          <input
            ...
            className={`w-12 h-14 text-center text-xl font-bold rounded-xl border-2 outline-none transition-all duration-200 ${
              digit
                ? mode === 'admin' ? 'border-red-400 bg-red-50 text-red-700' : 'border-primary bg-orange-50 text-primary'
      ...
      <div className="flex gap-2 mb-8">
        {pin.map((digit, index) => (
          <div key={index} className={`w-2 h-2 rounded-full transition-all duration-200 ${digit ? (mode === 'admin' ? 'bg-red-500' : 'bg-primary') : 'bg-gray-300'}`} />
        ))}
      </div>

      <button onClick={switchMode} className="text-sm text-gray-500 hover:text-gray-700 transition-colors underline decoration-dotted underline-offset-4">
